...
function meu_callback(conteudo) {
	...
	if (!("erro" in conteudo)) {
		...
		buscarArmazem(conteudo.codigodetalhe);
		
	} //end if.
	else {
		//produto não Encontrado.
		limpa_formulario();
		$.Notification.autoHideNotify('black', 'top center', 'Fecha em 5 seconds...','Produto não disponível Estoque!');
	}
}
...
function buscarproduto(valor) {
	...
	if (valor != "") {
		...
		script.src = 'jsonProdutoEstoque.php?/ws/callback=meu_callback&prd='+valor;
