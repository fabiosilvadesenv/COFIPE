                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="page-title-box">
                                    <h4 class="page-title">Conta(s) Despesa(s)</h4>
                                ...
                                <div class="card-box">
                                    ...
                                    <div class="row">
										...
										<p class="text-muted font-10 m-b-7">
											<?php if ($acaoNovo == 1){ ?>
												<button class="btn btn-success waves-effect waves-light btn-sm m-b-1" onclick="location.href='form-conta-Despesa-cadastro.php';"  >
														...
														try{
															$sql = "SELECT 	cr.id_conta_despesa, 
                                                                    ...
                                                                    FROM public.tb_conta_Despesa cr
                                                                        left join tb_lancamento lc on lc.id_conta_Despesa = cr.id_conta_Despesa 
                                                                    where st_excluido = 'N'
                                                                    group by cr.id_conta_despesa, 
                                                                            cr.tx_nome, 
                                                                            cr.tx_descricao,
                                                                            cr.dt_cadastro, 
                                                                            cr.st_ativo"; 
																...
																if($rs->rowCount() > 0){
																	while($row = $rs->fetch(PDO::FETCH_OBJ)){														
																		...
																		echo "	<td class=\"actions\">";
																				if ($acaoEditar == 1){																	
																					echo "		<button title=\"Editar Login!\"	onClick='javascript:editar(\"$row->id_conta_despesa\")'	class=\"btn btn-icon waves-effect waves-light btn-warning m-b-1\"> 	<i class=\"fa fa-pencil\"></i> 		</button>";
																				}
																				if ($acaoExcluir == 1){
																					echo "		<button title=\"Excluir Login!\" onClick='javascript:apagar(\"$row->id_conta_despesa\",\"$row->tx_nome\")' class=\"btn btn-icon waves-effect waves-light btn-danger m-b-1\"> 	<i class=\"fa fa fa-trash-o\"></i> 	</button>";
																				}															
																		echo "	</td>";
																		#is_numeric($id)
        ...
        <script type="text/javascript">
			...
			function apagar(codigo, nome){
				if (window.confirm('Deseja apagar a conta: "' + nome +'"?')) {  
				  $.ajax({  url: 'conta-Despesa-cadastro-u.php?act=d&rd='+codigo,
					type:'GET',
						data: $('#form').serialize(),  
			...
			function editar(codigo){
				location.href = 'form-conta-Despesa-cadastro.php?codigo='+codigo ;
			}
	
            function saldo(codigo){
				location.href = 'form-conta-Despesa-saldo-lista.php?codigo='+codigo ;
			}
