                                ...
                                <div class="card-box">
                                    ...
                                    <div class="row">
										...
										<p class="text-muted font-10 m-b-7">
											<?php if ($acaoNovo == 1){  
                                                $hrefBtnNovo = "'form-conta-receita-saldo-cadastro.php?codigoconta=$contaReceita';";  
                                            ?>
												
                                                <button class="btn btn-success waves-effect waves-light btn-sm m-b-1" onclick="location.href= <?php echo $hrefBtnNovo; ?>"  >
										...
										<table id="responsive-datatable" class="table table-bordered table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
													<thead>
														<tr>
															<th>Nome da Conta Crédito</th> 
															<th>Data</th> 
															<th>Descrição do Saldo</th>
                                                            <th>R$ Valor</th>
                                                            <th>Ação</th>
														</tr>
													</thead>
													<tbody>													
														...
														try{
															$sql = "SELECT 	sd.id_conta_receita_saldo, 
                                                                            ...
                                                                            sd.tx_descricao, 
                                                                            sd.vl_valor
                                                                    FROM public.tb_conta_receita_saldo sd
                                                                        inner join tb_conta_receita ct on ct.id_conta_receita = sd.id_conta_receita 
                                                                    where sd.id_conta_receita = :id_conta_receita
                                                                    "; 
  																		
																	$rs = $db->prepare($sql);

                                                                    $rs->bindValue(':id_conta_receita', $contaReceita);
																...
																if($rs->rowCount() > 0){
																	while($row = $rs->fetch(PDO::FETCH_OBJ)){														
																		...
																		echo "	<td class=\"actions\">";
																				if ($acaoEditar == 1){																	
																					echo "		<button title=\"Editar Saldo!\"	onClick='javascript:editar(\"$row->id_conta_receita_saldo\",\"$row->id_conta_receita\")'	class=\"btn btn-icon waves-effect waves-light btn-warning m-b-1\"> 	<i class=\"fa fa-pencil\"></i> 		</button>";
																				}
																				if ($acaoExcluir == 1){
																					echo "		<button title=\"Excluir Saldo!\" onClick='javascript:apagar(\"$row->id_conta_receita_saldo\",\"$row->tx_nome\")' class=\"btn btn-icon waves-effect waves-light btn-danger m-b-1\"> 	<i class=\"fa fa fa-trash-o\"></i> 	</button>";
																				}															
																		echo "	</td>";
																		#is_numeric($id)
        ...
        <script type="text/javascript">
			...
			function apagar(codigo, nome){
				if (window.confirm('Deseja apagar a conta: "' + nome +'"?')) {  
				  $.ajax({  url: 'conta-receita-cadastro-saldo-u.php?act=d&rd='+codigo,
					type:'GET',
						data: $('#form').serialize(),  
			...
			function editar(codigosaldo, codigoconta){
				location.href = 'form-conta-receita-saldo-cadastro.php?codigosaldo='+codigosaldo+'&codigoconta='+codigoconta ;
			}
