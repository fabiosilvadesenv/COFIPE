                                ...
                                <div class="card-box">
                                    ...
                                    <div class="row">
										...
										<p class="text-muted font-10 m-b-7">
											<?php if ($acaoNovo == 1){ ?>
												<button class="btn btn-success waves-effect waves-light btn-sm m-b-1" onclick="location.href='form-login-cadastro.php';"  >
										...
										<table id="responsive-datatable" class="table table-bordered table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
													...
													<tbody>													
														...
														try{
															$sql = "SELECT 
																	  lg.id_login,
																	  case when lg.st_administrador = 'S' then 'Administrador' else 'Usuário' end as tx_login_tipo,
																	  lg.tx_primeiro_nome,
																	  lg.tx_segundo_nome,
																	  lg.tx_login,
																	  fnc_dominio_s_n(LG.st_ativo) st_ativo
																	FROM tb_login lg
                                                                    where st_excluido = 'N'";
															...
															if($rs->execute()){
																if($rs->rowCount() > 0){
																	while($row = $rs->fetch(PDO::FETCH_OBJ)){														
																		...
																		echo "<tr>";
																		echo "	<td>$row->tx_primeiro_nome</td>";
																		echo "	<td>$row->tx_segundo_nome</td>";
																		echo "	<td>$row->tx_login</td>";
																		echo "	<td>$row->tx_login_tipo</td>";
																		echo "	<td>$row->st_ativo</td>";																		
																		echo "	<td class=\"actions\">";
																				if ($acaoEditar == 1){																	
																					echo "		<button title=\"Editar Login!\"	onClick='javascript:editar(\"$row->id_login\")'	class=\"btn btn-icon waves-effect waves-light btn-warning m-b-1\"> 	<i class=\"fa fa-pencil\"></i> 		</button>";
																				}
																				if ($acaoExcluir == 1){
																					echo "		<button title=\"Excluir Login!\" onClick='javascript:apagar(\"$row->id_login\",\"$row->tx_login\")' class=\"btn btn-icon waves-effect waves-light btn-danger m-b-1\"> 	<i class=\"fa fa fa-trash-o\"></i> 	</button>";
																				}
                                                                                if ($acaoEditar == 1){																	
																					echo "		<button title=\"Trocar Senha!\"	onClick='javascript:editarSenha(\"$row->id_login\")'	class=\"btn btn-icon waves-effect waves-light btn-secondary m-b-1\"> <i class=\"fa fa-wrench\"></i> </button>";
																				}																
																		echo "	</td>";
																		#is_numeric($id)
       ...
       <script>
        ...
        <script type="text/javascript">
			...
			function apagar(codigo, nome){
				if (window.confirm('Deseja apagar o Login: "' + nome +'"?')) {  
				  $.ajax({  url: 'login-lista-cadastro-u.php?act=d&rd='+codigo,
					type:'GET',
						data: $('#form').serialize(),  
			...
			function editar(codigo){
				location.href = 'form-login-cadastro.php?codigo='+codigo ;
			}
            
            function editarSenha(codigo){
				location.href = 'form-atualizar-senha.php?codigo='+codigo ;
			}
